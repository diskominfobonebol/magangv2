...
<head>
    ...
    <title>Buat Kata Sandi Baru - Mahasiswa Magang | SIMPATIK</title>
...
<body class="app-background min-h-screen flex items-center justify-center p-4 antialiased">
    <div class="max-w-md w-full" x-data="{ showPass: false, showConfirm: false }">
        <!-- Logo & Header -->
        <div class="text-center mb-8">
            ...
            <h2 class="mt-3 text-2xl font-bold text-navy">Buat Kata Sandi Baru</h2>
            <p class="mt-1 text-sm text-slate-600 font-medium">Silakan tentukan kata sandi baru untuk akun magang Anda</p>
        </div>
        ...
        <div class="bg-card-gradient p-8 sm:p-10 rounded-3xl shadow-xl shadow-blue-900/5 border border-blue-200/50 backdrop-blur-sm">
            ...
            <form action="{{ route('password.update') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input id="email" name="email" type="email" autocomplete="email" required class="form-input bg-slate-50" value="{{ $email ?? old('email') }}" readonly>
                </div>

                <div>
                    <label for="password" class="form-label">Kata Sandi Baru</label>
                    <div class="relative">
                        <input :type="showPass ? 'text' : 'password'" id="password" name="password" required class="form-input pr-10" placeholder="Minimal 6 karakter">
                        <button type="button" @click="showPass = !showPass" class="absolute inset-y-0 right-0 flex items-center pr-3.5 text-slate-400 hover:text-[#3B82F6]">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input :type="showConfirm ? 'text' : 'password'" id="password_confirmation" name="password_confirmation" required class="form-input pr-10" placeholder="Ulangi kata sandi baru">
                        <button type="button" @click="showConfirm = !showConfirm" class="absolute inset-y-0 right-0 flex items-center pr-3.5 text-slate-400 hover:text-[#3B82F6]">
